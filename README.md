# Game Key Web Store Frontend

The frontend client for the e-commerce game key web store, built using Angular. It features dynamic product filtering, cart management, and client-side routing.

## Features

* **Product Catalog**: Browsing and filtering available game keys.
* **Shopping Cart**: Manage selected items and order checkout process.
* **Component-Driven Architecture**: Structured using modular components.
* **Routing**: Client-side routing for seamless page navigation.

## How to Run

### Prerequisites
* Node.js and npm installed.

### Installation and Execution

1. Install project dependencies:
   ```bash
   npm install
   ```

2. Run the local development server:
   ```bash
   npm start
   ```
   Open `http://localhost:4200` in your browser.
